...
function archivoCargado() {
    ...
    function ArmaGridMapa(Datos) {
        ...
        $("#gvMapas").kendoGrid({
            ...
            columns: [
                ...
                {
                    template: " #= TempleteBorrarMapa(mapaId) #",
                    title: "",
                    width: "10%"
                }
            ]
    ...
    function TempleteBorrarMapa(idMapa) {
        var strTemplate = "<input type='button' class='btn btn-primary ' value='Borrar' onclick='BorrarMapa(" + idMapa + ");'/>";
        return strTemplate;
    }



    function BorrarMapa(idMapa) {
        spinner();
        $.ajax({
            type: "POST",
            url: "../Administracion/BorrarMapa",
            data: { "idMapa": idMapa },
            cache: false,
            success: function (result) {
                if (result.ProcesoExitoso == 1) {
                    ObtenerRegistrosSeguimiento();
                    MostrarMensajeExito("El mapa se ha borrado correctamente.");
                } else {
